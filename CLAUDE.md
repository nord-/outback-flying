...
- **`data/airports.ts`** & **`data/aircraft.ts`** — static catalogues. Airports carry a `type` tier (`hub | regional | strip` — a game role, not a physical classification), real physical data (`runwayM`, `surface`, `lighted` mostly from OurAirports; East African strips' runway length and surface from OpenStreetMap under ODbL, © OpenStreetMap contributors; a few other fields hand-filled from other sources and noted in-file), and `fuelTypes` + `fuelPriceMult`. Aircraft carry `minRunwayM` and are looked up by `specId` via `getSpec()`.
